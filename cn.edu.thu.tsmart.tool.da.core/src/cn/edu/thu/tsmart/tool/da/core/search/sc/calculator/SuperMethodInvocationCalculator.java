...
public class SuperMethodInvocationCalculator {
	public static double sim(SuperMethodInvocation e1, Expression e2) {
		...
		// }
		// else
		// if(e2 instanceof SuperFieldAccess){
		// return sim(e1,(SuperFieldAccess)e2);
		// }
		// else
		if (e2 instanceof SuperMethodInvocation) {
	...
	public static double sim(SuperMethodInvocation e1, SuperMethodInvocation e2) {
		double methodNameSimilarity = U.stringSim(e1.getName().toString(), e2
				.getName().toString());
		double argumentSizeSimilarity = U.intSim(e1.arguments().size(), e2
				.arguments().size());
		return U.mean(methodNameSimilarity, argumentSizeSimilarity);
	}
}
