...
public class ArrayAccessCalculator {

	public static double sim(ArrayAccess e1, Expression e2) {
	...
	public static double sim(ArrayAccess e1, ArrayAccess e2) {
		Expression a1 = e1.getArray(), a2 = e2.getArray(), i1 = e1.getIndex(), i2 = e2
				.getIndex();
		// array可能是啥(值为数组类型的任何表达式?), index可能是啥(值为整数或可隐式转换成整数的任何表达式?)...
		double arraySimilarity = SimilarityCalculator.sim(a1, a2);
		double indexSimilarity = SimilarityCalculator.sim(i1, i2);
		return U.mean(arraySimilarity, indexSimilarity);
	}
}
