...
public class ClassInstanceCreationCalculator {
	public static double sim(ClassInstanceCreation e1, Expression e2) {
		...
		// }
		// else
		if (e2 instanceof ClassInstanceCreation) {
	...
	public static double sim(ClassInstanceCreation e1, ClassInstanceCreation e2) {
		// e1.getType() 包括类型参数 T<T,T,...> 这样
		double typeSimilarity = U.stringSim(e1.getType().toString(), e2
				.getType().toString());
		double argumentSizeSimilarity = U.intSim(e1.arguments().size(), e2
				.arguments().size());
		return U.mean(typeSimilarity, argumentSizeSimilarity);
	}
}
