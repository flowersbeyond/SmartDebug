...
public class PrefixExpressionCalculator {
	public static double sim(PrefixExpression e1, Expression e2){
...
//        }
	...
	public static double sim(PrefixExpression e1, PrefixExpression e2){
        if(U.canPrefixOpReturnBoolean(e1.getOperator()) && U.canPrefixOpReturnBoolean(e2.getOperator())){
        	return 1;
        }
        else if(!U.canPrefixOpReturnBoolean(e1.getOperator()) && !U.canPrefixOpReturnBoolean(e2.getOperator())){
        	return U.doubleSim(Double.parseDouble(e1.getOperand().toString()), Double.parseDouble(e2.getOperand().toString()));
        }
        else{
        	return 0;
        }
