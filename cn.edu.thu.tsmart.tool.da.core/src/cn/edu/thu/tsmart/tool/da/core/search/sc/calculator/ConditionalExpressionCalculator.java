...
public class ConditionalExpressionCalculator {
	public static double sim(ConditionalExpression e1, Expression e2) {
		...
		// }
		// else
		if (e2 instanceof ConditionalExpression) {
	...
	public static double sim(ConditionalExpression e1, ConditionalExpression e2) {
		double conditionSimilarity = SimilarityCalculator.sim(
				e1.getExpression(), e2.getExpression());
		double thenSimilarity = SimilarityCalculator.sim(
				e1.getThenExpression(), e2.getThenExpression());
		double elseSimilarity = SimilarityCalculator.sim(
				e1.getElseExpression(), e2.getElseExpression());
		return U.mean(conditionSimilarity, thenSimilarity, elseSimilarity);
	}
}
